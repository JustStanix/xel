  ...
  <template>
    <style>
      :host {
        ...
        --arrow-d: path(
          "M 11.7 19.9 L 49.8 57.9 L 87.9 19.9 L 99.7 31.6 L 49.8 81.4 L -0.01 31.6 Z"
        );
...
export class XTabElement extends HTMLElement {
  ...
  _updateArrowVisibility() {
    ...
    this["#arrow"].style.display =
      menu === null && popover === null ? "none" : null;
  }
  ...
  async _onPointerDown(pointerDownEvent) {
    ...
    if (
      ...
      !pointerDownEvent.target.closest("x-menu, x-popup")
    ) {
  ...
  async _onClick(event) {
    // Ripple
    if (
      this["#ripples"].querySelector(".pointer-down-ripple") === null &&
      !this.querySelector("x-menu")
    ) {
      let triggerEffect = getComputedStyle(this)
        .getPropertyValue("--trigger-effect")
        .trim();
