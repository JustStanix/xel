...
export class XTabsElement extends HTMLElement {
  ...
  _expand(tab) {
    return new Promise(async (resolve) => {
      ...
      if (menu) {
        this._wasFocusedBeforeExpanding =
          this.querySelector("*:focus") !== null;

        let over =
          getComputedStyle(tab).getPropertyValue("--menu-position").trim() ===
          "over";
        let whenOpened = over
          ? menu.openOverLabel(label)
          : menu.openNextToElement(tab, "vertical", 3);
  ...
  _animateSelectionIndicator(startTab, endTab) {
    return new Promise(async (resolve) => {
      ...
      this[
        "#selection-indicator"
      ].style.height = computedStyle.getPropertyValue(
  ...
  _onKeyDown(event) {
    if (event.code === "Enter" || event.code === "Space") {
      ...
      if (menu) {
        ...
        if (menu.opened) {
          let lastMenuItem = menu.querySelector(
            ":scope > x-menuitem:last-of-type:not([disabled])"
          );
      ...
      if (menu) {
        ...
        if (menu.opened) {
          let firstMenuItem = menu.querySelector(
            ":scope > x-menuitem:not([disabled])"
          );
